...
require \prelude-ls
{dasherize, filter, find, fold, keys, map, concat-map, obj-to-pairs, Obj, id, pairs-to-obj, sort-by, unique-by, each, all, any, is-type} = require \prelude-ls
...
create-livescript-editor = (element-id)->
    editor = ace.edit element-id
        ...
        ..commands.on \afterExec (e) ->
            # d = e.editor
            # r = d.getSelectionRange()
            # r.clone().setStart(r.start.row, 0)
            # d.session.getTextRange(r)
...
$ ->
    ...
    do ->
        ...
        collection-keywords <- $.post "/keywords/queryContext", JSON.stringify {server-name, database, collection}
